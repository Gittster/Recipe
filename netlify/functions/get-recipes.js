// functions/get-recipes.js
// Read-only recipe library listing for the SweetSuite dashboard's Recipes
// browser (as opposed to get-recipe.js, which fetches one full recipe).
// Not used by the recipe app itself.
console.log("get-recipes.js: Loading modules.");
...
if (!admin.apps.length) {
    try {
        ...
        console.error("get-recipes.js: Failed to initialize Firebase Admin:", initError);
    }
} else {
    firebaseReady = true;
}
...
exports.handler = async (event) => {
    ...
    if (!SWEETSUITE_API_KEY) {
        console.error("get-recipes.js: SWEETSUITE_API_KEY is not configured.");
    ...
    try {
        ...
        const snapshot = await admin.firestore().collection('recipes').where('uid', '==', uid).get();
        ...
        console.error("get-recipes.js: Failed to fetch recipes:", queryError);
        return { statusCode: 500, headers, body: JSON.stringify({ error: 'Failed to fetch recipes.' }) };
    }
};
